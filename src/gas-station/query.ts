...
const main = async () => {
    ...
    const gasStationPrefix = '0xd37dc4d7e2c1bdf3edd89db0e505394ea69af43d/gas-station/';
    ...
    for (const streamId of streamList) {
        ...
        const response = await client.query(streamId, {last: 1});
        ...
        for await (const event of response) {
            ...
            const fastInGwei = ethers.utils.formatUnits(llamaRpcData.fast.maxFee, 'gwei');

            const networkName = streamId.split('/').slice(-1)[0];

            console.log(`Fast fee on ${networkName} at block ${llamaRpcData.blockNumber} is ${fastInGwei} gwei`);
        }

        // if you want some real time data, you can subscribe to the stream
        const subscription = await client.subscribe(streamId);

        subscription.useLegacyOnMessageHandler(content => {
            // @ts-ignore - just for demonstration purposes, we won't maintain a type for this
